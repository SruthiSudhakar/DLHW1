    ...
    for it in range(num_iters):
      ...
      for p in model:
        ...
        if update == 'sgd':
          ...
          #####################################################################
          dx =  grads[p]
          self.step_cache[p] = self.step_cache[p]* momentum + dx
          dx = -learning_rate * self.step_cache[p]
          #####################################################################
          #                      END OF YOUR CODE                             #
          #####################################################################
        elif update == 'rmsprop':
          ...
          try:
              dx =  grads[p]
              self.step_cache[p] = learning_rate_decay * self.step_cache[p] + (1-learning_rate_decay) * dx * dx
